...
export class TestECPClient {
  ...
  constructor(options: TestECPClientOptions = {}) {
    ...
    if (this.captureNotifications) {
      ...
      this.gitAdapter.setNotificationHandler((notification) => {
        this.notifications.push(notification);
      });
    }
  }
  ...
  async shutdown(): Promise<void> {
    ...
    // Dispose git service resources
    this.gitAdapter.dispose();
   ...
   */
  private getAdapterForMethod(
    method: string
  ): DocumentServiceAdapter | FileServiceAdapter | GitServiceAdapter | null {
